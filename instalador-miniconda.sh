...
echo -e "${GREEN}"
echo "----------------------------------------------------------"
echo "           Instalação do Miniconda - Versão 2.0           "
echo "----------------------------------------------------------"
echo -e "${NC}"
...
# Verifica se o diretório de instalação já existe
check_existing_installation() {
    if [ -d "$INSTALL_DIR" ]; then
        echo -e "${YELLOW}Aviso: O diretório de instalação $INSTALL_DIR já existe.${NC}"
        read -p "Deseja continuar? (A instalação anterior será sobrescrita) [s/N]: " resposta
        if [[ ! "$resposta" =~ ^[Ss]$ ]]; then
